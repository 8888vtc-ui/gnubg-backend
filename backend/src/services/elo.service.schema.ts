...
export class EloService {
  ...
  public static async updateEloRatings(result: EloResult): Promise<void> {
    try {
      ...
      // Update database (schema doesn't have updatedAt field)
      await Promise.all([
        prisma.users.update({
  ...
  public static async getGlobalRankings(limit: number = 100, offset: number = 0): Promise<{
    ...
    try {
      // Get all active users (schema uses 'isActive')
      ...
      const rankings: RankingEntry[] = await Promise.all(
        users.map(async (user, index) => {
          // Count games using schema relations
          ...
          const gamesPlayed = whiteGames + blackGames;

          // Count wins (simplified - would need to query game results)
          const wins = Math.floor(gamesPlayed * 0.4); // Placeholder
          ...
          return {
            ...
            country: undefined // Schema doesn't have country field
          };
        })
      );

      // Get total count
      const total = await prisma.users.count({ where: { isActive: true } });
  ...
  public static async getUserRank(userId: string): Promise<{
    ...
    try {
      ...
      const totalPlayers = await prisma.users.count({ where: { isActive: true } });
  ...
  public static async getEloDistribution(): Promise<{
    ...
    try {
      const users = await prisma.users.findMany({
        where: { isActive: true }, // Schema uses 'isActive'
        select: { elo: true }
      });
