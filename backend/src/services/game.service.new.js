...
class GameService {
    ...
    static createGame(userId, gameData) {
        try {
            ...
            const player = {
                id: userId,
                color: types_1.SimplePlayerColor.WHITE,
                isHuman: true
            };
            const opponent = {
                id: 'ai_' + Date.now(),
                color: types_1.SimplePlayerColor.BLACK,
                isHuman: false
            };
            const initialBoard = GameService.createInitialBoard();
            const gameState = {
                id: GameService.generateGameId(),
                status: types_1.SimpleGameStatus.WAITING,
                mode: gameData.mode,
                isRanked: gameData.isRanked,
                timeLimit: gameData.timeLimit,
                currentPlayer: types_1.SimplePlayerColor.WHITE,
            ...
            utils_1.Logger.info('Game created successfully', {
                gameId: gameState.id,
                userId,
                action: 'game_created'
            });
            ...
            utils_1.Logger.error('Failed to create game', {
                userId,
    ...
    static rollDice(gameState) {
        ...
        utils_1.Logger.debug('Dice rolled', {
            gameId: gameState.id,
            dice: { die1, die2 }
        });
        return updatedState;
    ...
    static makeMove(gameState, moveData) {
        try {
            ...
            if (GameService.isTurnComplete(updatedState)) {
                updatedState.currentPlayer = updatedState.currentPlayer === types_1.SimplePlayerColor.WHITE
                    ? types_1.SimplePlayerColor.BLACK
                    : types_1.SimplePlayerColor.WHITE;
                updatedState.dice = { die1: 0, die2: 0 };
            }
            // Vérifier la victoire
            const winner = GameService.checkWinner(updatedState);
            if (winner) {
                updatedState.status = types_1.SimpleGameStatus.COMPLETED;
                updatedState.winner = winner;
            }
            utils_1.Logger.info('Move executed successfully', {
                gameId: gameState.id,
                move: moveData
            });
            return updatedState;
            ...
            utils_1.Logger.error('Failed to execute move', {
                gameId: gameState.id,
                action: 'move_failed',
                error: error instanceof Error ? error.message : 'Unknown error'
            });
    ...
    static getGameState(gameId) {
        try {
            utils_1.Logger.debug('Fetching game state', { gameId, action: 'fetch_game_state' });
            return null;
        }
        catch (error) {
            utils_1.Logger.error('Failed to fetch game state', {
                gameId,
    ...
    static createInitialBoard() {
        ...
        board[1] = { point: 1, checkers: 2, player: types_1.SimplePlayerColor.WHITE };
        board[12] = { point: 12, checkers: 5, player: types_1.SimplePlayerColor.WHITE };
        board[17] = { point: 17, checkers: 3, player: types_1.SimplePlayerColor.WHITE };
        board[19] = { point: 19, checkers: 5, player: types_1.SimplePlayerColor.WHITE };
        board[24] = { point: 24, checkers: 2, player: types_1.SimplePlayerColor.BLACK };
        board[13] = { point: 13, checkers: 5, player: types_1.SimplePlayerColor.BLACK };
        board[8] = { point: 8, checkers: 3, player: types_1.SimplePlayerColor.BLACK };
        board[6] = { point: 6, checkers: 5, player: types_1.SimplePlayerColor.BLACK };
        return board;
    }
    /**
    ...
    static isValidMove(gameState, move) {
        ...
        const direction = currentPlayer === types_1.SimplePlayerColor.WHITE ? 1 : -1;
    ...
    static executeMove(board, move) {
        ...
        if (toPoint.player && toPoint.player !== fromPoint.player && toPoint.checkers === 1) {
            ...
            newBoard[0].checkers++;
            newBoard[0].player = toPoint.player === types_1.SimplePlayerColor.WHITE ? types_1.SimplePlayerColor.BLACK : types_1.SimplePlayerColor.WHITE;
        }
        else {
            toPoint.checkers++;
            toPoint.player = fromPoint.player;
...
//# sourceMappingURL=game.service.new.js.map